...
const ephemeralOutputRoot = path.resolve(process.env.EPHEMERAL_OUTPUT_DIR || path.join(localOutputDir, 'downloads'));
const storageAvailable = true; // Converted files are ephemeral: temp file -> stream to user -> delete.
...
function ephemeralStoragePathFromKey(key) {
  const cleanKey = String(key || '').replace(/^ephemeral:/, '');
  const resolved = path.resolve(ephemeralOutputRoot, cleanKey);
  if (resolved !== ephemeralOutputRoot && !resolved.startsWith(`${ephemeralOutputRoot}${path.sep}`)) throw new Error('Invalid ephemeral output path.');
  return resolved;
}

async function streamEphemeralDownload(res, file, filename, contentType = 'application/octet-stream') {
  ...
  const remove = async () => { await unlink(file).catch(() => {}); };
  stream.on('error', async () => { await remove(); if (!res.headersSent) res.status(500); else res.destroy(); });
...
app.get('/api/health', async (_req, res) => {
  ...
  res.json({ ok: redis === 'ok', service: 'convertly-api', redis, storage: 'ephemeral-stream', timestamp: new Date().toISOString() });
});
...
app.get('/api/conversion/:id', requireUser, async (req, res) => {
  ...
  if (req.isGuest) {
    ...
    const progress = typeof job.progress === 'number' ? job.progress : Number(meta?.progress || 0);
...
app.post('/api/conversion/:id/share', requireUser, async (req, res) => {
  ...
  try {
    ...
    if (req.isGuest) {
      ...
      const { data, error } = await supabaseAdmin.from('conversions').select('id,status,output_path,download_expires_at,format').eq('id', id).eq('user_id', req.user.id).maybeSingle();
...
app.get('/api/share/:token', async (req, res) => {
  try {
    ...
    if (share.guest) {
      ...
      const file = path.resolve(job.returnvalue);
      if (!file.startsWith(`${ephemeralOutputRoot}${path.sep}`)) return res.status(403).json({ error: 'Invalid shared download path.' });
      const ok = await streamEphemeralDownload(res, file, `convertly-${share.id}.${job.data.format === 'mp3' ? 'mp3' : 'mp4'}`, job.data.format === 'mp3' ? 'audio/mpeg' : 'video/mp4');
      if (!ok) return res.status(410).json({ error: 'This shared conversion file is no longer available.' });
      return;
    }
    const { data, error } = await supabaseAdmin.from('conversions').select('id,status,output_path,download_expires_at,format').eq('id', share.id).eq('user_id', share.userId).maybeSingle();
    ...
    if (data.output_path.startsWith('ephemeral:')) {
      const file = ephemeralStoragePathFromKey(data.output_path);
      const ok = await streamEphemeralDownload(res, file, `convertly-${data.id}.${data.format === 'mp3' ? 'mp3' : 'mp4'}`, data.format === 'mp3' ? 'audio/mpeg' : 'video/mp4');
      if (!ok) return res.status(410).json({ error: 'This shared conversion file is no longer available.' });
      return;
    }
    return res.status(410).json({ error: 'This shared conversion uses an old storage mode. Please run the conversion again.' });
  } catch (error) {
    console.error(error);
...
app.get('/api/download/:id', requireUser, async (req, res) => {
  try {
    ...
    if (req.isGuest) {
      ...
      if (await redis.get(`convertly:cancel:${id}`)) return res.status(409).json({ error: 'Conversion was cancelled.' });
      if (state !== 'completed' || !file) return res.status(409).json({ error: 'Conversion is not ready.' });
      const safeFile = path.resolve(file);
      if (safeFile !== file || (!safeFile.startsWith(`${ephemeralOutputRoot}${path.sep}`))) return res.status(403).json({ error: 'Invalid download path.' });
      const ok = await streamEphemeralDownload(res, safeFile, `convertly-${id}.${job.data.format === 'mp3' ? 'mp3' : 'mp4'}`, job.data.format === 'mp3' ? 'audio/mpeg' : 'video/mp4');
      if (!ok) return res.status(410).json({ error: 'Download expired. Please run the conversion again.' });
      return;
    }
    const { data, error } = await supabaseAdmin.from('conversions').select('id,status,output_path,download_expires_at,format').eq('id', id).eq('user_id', req.user.id).maybeSingle();
    if (error || !data) return res.status(404).json({ error: 'Conversion not found.' });
    ...
    if (!data.output_path.startsWith('ephemeral:')) return res.status(410).json({ error: 'This conversion uses an old storage mode. Please run the conversion again.' });
    const file = ephemeralStoragePathFromKey(data.output_path);
    const ok = await streamEphemeralDownload(res, file, `convertly-${id}.${data.format === 'mp3' ? 'mp3' : 'mp4'}`, data.format === 'mp3' ? 'audio/mpeg' : 'video/mp4');
    if (!ok) return res.status(410).json({ error: 'Download expired. Please run the conversion again.' });
    res.on('finish', () => { supabaseAdmin.from('conversions').update({ status: 'expired', output_path: null }).eq('id', id).eq('user_id', req.user.id).then(() => {}).catch(() => {}); });
  } catch (error) {
    console.error(error);
    if (!res.headersSent) res.status(500).json({ error: 'Could not stream the download.' });
    else res.destroy();
  }
});
...
app.get('/api/admin/overview', requireUser, async (req, res) => {
  try {
    ...
    res.json({ users: users.count || 0, activePro: pro.count || 0, conversions: conversions.count || 0, failed: failed.count || 0, conversionsToday: today.count || 0, topQuality: topQuality ? Number(topQuality) : null, limits: LIMITS, storage: 'ephemeral-stream', recentEvents: events.data || [] });
