...
const remoteStorageConfigured = false;
...
function run(cmd, args, onProgress, jobId) {
  return new Promise((resolve, reject) => {
    ...
    child.stderr.on('data', chunk => {
      ...
      if (matches?.length) onProgress?.(Number.parseFloat(matches[matches.length - 1]));
    });
...
function qualityFormat(q) {
  const max = q >= 2160 ? 2160 : q;
  // Prefer H.264 + AAC/MP4 audio so the resulting MP4 opens in Windows Media Player
  // and common phones/TVs without requiring extra codecs. A generic fallback is kept
  // for sources that do not expose an H.264 rendition; those are normalized below.
  return `bestvideo[height<=${max}][vcodec^=avc1]+bestaudio[acodec^=mp4a]/bestvideo[height<=${max}][vcodec^=avc1]+bestaudio/best[height<=${max}][vcodec^=avc1][acodec^=mp4a]/best[height<=${max}]`;
}

async function probeCodecs(input) {
  const raw = await runCapture(process.env.FFPROBE_BIN || 'ffprobe', [
    '-v', 'error', '-select_streams', 'v:0', '-show_entries', 'stream=codec_name',
    '-of', 'default=noprint_wrappers=1:nokey=1', input
  ]).catch(() => '');
  const audio = await runCapture(process.env.FFPROBE_BIN || 'ffprobe', [
    '-v', 'error', '-select_streams', 'a:0', '-show_entries', 'stream=codec_name',
    '-of', 'default=noprint_wrappers=1:nokey=1', input
  ]).catch(() => '');
  return { video: raw.trim().split('\n')[0] || '', audio: audio.trim().split('\n')[0] || '' };
}

async function normalizeMp4Compatibility(input, output, onProgress, jobId) {
  const codecs = await probeCodecs(input);
  const force = String(process.env.FORCE_COMPATIBLE_MP4 ?? 'true').toLowerCase() === 'true';
  if (!force || (codecs.video === 'h264' && (!codecs.audio || codecs.audio === 'aac'))) {
    await import('node:fs/promises').then(m => m.copyFile(input, output));
    return;
  }
  await run(process.env.FFMPEG_BIN || 'ffmpeg', [
    '-y', '-i', input,
    '-map', '0:v:0', '-map', '0:a:0?',
    '-c:v', 'libx264', '-preset', process.env.COMPATIBILITY_PRESET || 'veryfast', '-crf', process.env.COMPATIBILITY_CRF || '18',
    '-c:a', 'aac', '-b:a', process.env.COMPATIBILITY_AUDIO_BITRATE || '192k',
    '-movflags', '+faststart', '-progress', 'pipe:2', output
  ], p => onProgress?.(Math.min(99, 90 + Math.round((p / 100) * 9))), jobId);
}

console.log(`Ephemeral output mode enabled. Files are deleted after download or TTL (${Number(process.env.DOWNLOAD_TTL_MINUTES || 30)} minutes).`);
...
const worker = new Worker('convertly-conversions', async job => {
  ...
  try {
    ...
    if (!isMp3 && targetSizeMB) {
      ...
      await normalizeMp4Compatibility(rawOutput, output, async p => {
        await job.updateProgress(p).catch(() => {});
        const meta = await updateProgressMeta(jobId, p, startedAt).catch(() => null);
        await updateBatch(batchId, batchIndex, { status: 'processing', progress: p, etaSeconds: meta?.etaSeconds ?? null }).catch(() => {});
        if (!guest && supabase) await supabase.from('conversions').update({ progress: p }).eq('id', jobId).eq('user_id', userId);
    ...
    // Ephemeral delivery model: keep the finished file only on the backend's
    // temporary filesystem until the user downloads it, then delete it.
    await mkdir(ephemeralOutputRoot, { recursive: true });
    const persistentOutput = path.join(ephemeralOutputRoot, `${jobId}.${extension}`);
    ...
    const outputKey = `ephemeral:${jobId}.${extension}`;
    if (supabase && !guest) {
      await supabase.from('conversions').update({ status: 'completed', progress: 100, output_path: outputKey, download_expires_at: expires, completed_at: new Date().toISOString() }).eq('id', jobId).eq('user_id', userId);
    }
